...
if __name__ == "__main__":
    sim=simulation("wall")
    sim.q0 = [0 , -np.pi/1.3, np.pi/1.5, -np.pi/4, np.pi,-np.pi/1.5,np.pi/1.4 , np.pi/2, -np.pi/2,0]  # 0, -3*np.pi/4, np.pi/3, np.pi, 0, 0, np.pi/3 , 0, 0,0] #home pose
    ...
    OP_vel_controller = OP_Space_Velocity_controller(kd_trans=400,kd_ori=40, kp_trans=250,kp_ori=300,Kv=50,lambdaTraj=0.5,lambdaAvoid=0.5)
